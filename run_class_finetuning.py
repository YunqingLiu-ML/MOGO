...
from engines.engine_for_finetuning import train_one_epoch, final_test
from utils import NativeScalerWithGradNormCount as NativeScaler
from utils import multiple_samples_collate
import utils
import contextlib
from models import *
from engines.matcher import HungarianMatcher
from engines.detr import SetCriterion

def get_args():
    parser = argparse.ArgumentParser('MOGO fine-tuning and evaluation script for video action detection', add_help=False)
...
def main(args, ds_init):
    ...
    dataset_test = build_dataset(is_train=False, test_mode=True, args=args)
    ...
    if args.finetune:
        ...
        pos_tokens = checkpoint_model['pos_embed'][:, 1:]
    ...
    if args.enable_deepspeed:
        ...
        if not args.no_amp:
            ...
            amp_autocast = lambda: torch.cuda.amp.autocast(dtype=dtype)
    ...
    matcher = HungarianMatcher(cost_class=11, cost_bbox=1, cost_giou=1)
    weight_dict = {'loss_ce': 11, 'loss_bbox': 1, 'loss_giou': 1}
    eos_coef = 0.1  # no-object
    losses = ['labels', 'boxes', 'cardinality']
    criterion = SetCriterion(22, matcher=matcher, weight_dict=weight_dict,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            print(f"[{timestep}] Accuracy of the network on the {len(dataset_val)} val videos: {test_stats['mAP_50']:.1f}%")
            if max_accuracy < test_stats["mAP_50"]:
                max_accuracy = test_stats["mAP_50"]
                if args.output_dir and args.save_ckpt:
                    utils.save_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch=epoch, model_name='best', model_ema=model_ema)

            print(f'Max mAP_50: {max_accuracy:.2f}%')
            if log_writer is not None:
                log_writer.update(val_acc1=test_stats['mAP_50_95'], head="perf", step=epoch)
                log_writer.update(val_acc5=test_stats['mAP_50'], head="perf", step=epoch)
                log_writer.update(val_acc5=test_stats['mAP_75'], head="perf", step=epoch)
    ...
    if global_rank == 0:
        print("Start merging results...")
        print(f"Accuracy of the network on the {len(dataset_test)} test videos:")
        print(f"Test mAP: {test_stats['mAP_50']:.4f}")
