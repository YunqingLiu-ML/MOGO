...
class NativeScalerWithGradNormCount:
    ...
    def __init__(self):
        self._scaler = torch.cuda.amp.GradScaler(init_scale=2**10)
...
def multiple_samples_collate(batch, fold=False):
    samples = torch.stack([b[0] for b in batch], dim=0)
    targets = []
    for b in batch:
        bboxes = b[1]
        labels = b[2]
        ...
        if bboxes.numel() == 0:
            ...
            if bboxes.ndim == 1 and bboxes.size(0) == 4:
        ...
        target = {'boxes': bboxes.float(), 'labels': labels}
    ...
    for idx, target in enumerate(targets):
        ...
        try:
            if boxes.numel() > 0:
                ...
                print(f"[Warning] Empty boxes at sample index {idx}. Skipping normalization.")
        except IndexError as e:
            print(f"[Error] IndexError while normalizing boxes at sample index {idx}: {e}")
        target['boxes'] = boxes
